...
/* ---------------------------------------------------------------------
 * Name der Klasse: ConsolenAnwendungJsoup
...
public class ConsolenAnwendungJsoup {
           ...
           if(!fhelp) {
               ...
               if(fproxy && fport && frt && fre && fdatei) { // schauen, ob alles validiert wurde
                   ...
                   if(isURL(url)) { // url testen,. wenn proxy benoetigt vorher erstellen
                       if (frtvor) bs = new BaueSeitenJsoup(url, null, re, irt, true);
                           else bs = new BaueSeitenJsoup(url, null, re, 0, false);
